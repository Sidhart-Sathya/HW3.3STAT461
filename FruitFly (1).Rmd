...
##Introduction 
The birds and the bees...well the flies to be exact. Fruit flies are that pesky little animal that most people want out of their home. But for researchers they serve as a crucial part in research. Fruit flies (drosophila melanogaster) are one of the most extensively studied organisms in biological research. They are most commonly used in biomedial research and allow us to better understand the human condition. The study being analyzed today is no different.
...
```
Now that we understand what we are looking at and what we hope to measure, we want to use ANOVA. In order to use ANOVA though certain assumptions must be met.
...
```
As we check for Homoscedasticity, we see there is an even distribution and no funneling. Thus, we proceed.
...
```
Since all observations are within the 90% envelope, we can proceed. 
...
```
As no pattern is seen, we can assume independence of observations. Now that all assumptions have been met, we can perform ANOVA.
...
```
From this we can see all the comparisons of pairs to evaluate how they compare to the GSAM. Right away, we can tell that the biggest differences lie between the comparison of eight virgin fruit flies vs. one virgin fruit fly. 
...
## Discussion
While the original study did not go into as much depth as our analysis, we can contuniue under the assumtpion that the pairwise combinations that were statistically significant were: Eight Pregnant vs. One Virgin, Eight Pregnant vs. One Pregnant, One Virgin vs. One Pregnant, Eight Pregnant vs. Null, One Virgin vs. Null, and One Pregnant vs. Null. 
